...
| v1.53 | 30-08-2026 | Stap 4A/4B gebouwd: eindbeslissing na het kennismakingsgesprek, welkomstmail en afwijzingsmail (CC-opdracht "Stap 4A/4B: eindbeslissing, welkomstmail & afwijzingsmail", `CC-opdracht_Stap4A_4B_Eindbeslissing-2.docx`). Nieuwe kolommen op `aanmeldingen`: `eindbeslissing` (`'match'` / `'geen_match'`), `eindbeslissing_op`, en `eindbeslissing_mail_verzonden_op` (migratie `20260830150000_aanmeldingen_eindbeslissing.sql`) — dat laatste veld stond niet in de opdracht maar is nodig om de gevraagde "foutafhandeling zoals in v1.32" daadwerkelijk te bouwen (mail_verzonden_op-patroon steunt op precies zo'n kolom). Twee nieuwe knoppen op de toetsingspagina, onder de bestaande knoppenrij (`EindbeslissingForm.tsx` + route `api/admin/aanmeldingen/[id]/eindbeslissing`): "Match — welkomstmail versturen" en "Geen match — afwijzen" — server-side niet terug te draaien (409 bij een tweede poging), zelfde foutafhandelingspatroon als v1.32. Nieuwe e-mailfuncties `welkomstEmail()` en `afwijzingEmailNaGesprek()` in nieuw bestand `src/lib/email/eindbeslissing.ts` (losstaand van de bestaande Stap 2-mails in `aanmeldingBeoordeling.ts`, die ongewijzigd blijven) — beide ondertekend met "Sabine Blok" i.p.v. "Team Private Yoga at Home" (bewust, volgt op een persoonlijk gesprek); `wrapEmailHtml()` in `emailLayout.ts` accepteert hiervoor nu een optionele `handtekeningHtml`-parameter (default ongewijzigd). `voornaam()`/`escapeHtml()`-helpers geëxtraheerd naar nieuw gedeeld bestand `emailHelpers.ts` (hergebruikt door beide mailbestanden, was gedupliceerd). Nieuwe `REGISTRATIE_LINK_PLACEHOLDER`-constante (zelfde patroon als `CALENDLY_LINK_PLACEHOLDER`, nog op `"#"` tot Stap 5 gebouwd is). Statuslabel-logica (`statusLabel()`, `aanmeldingenBeoordeling.ts`) uitgebreid: `eindbeslissing` heeft voorrang op `match_beslissing` zodra ingevuld → "Profiel uitgenodigd" resp. "Afgewezen (na gesprek)", zichtbaar in zowel het overzicht als de detailpagina. Omdat deze twee labels spaties/haakjes bevatten (geen geldige CSS-klassenaam via het bestaande `.toLowerCase()`-patroon), nieuwe expliciete `statusBadgeSlug()`-mapping toegevoegd. Beide nieuwe badges zijn bewust een outline-variant (alle 4 volle paletkleuren waren al vergeven aan de bestaande Stap 2/3-badges) — moskleurig voor "Profiel uitgenodigd", terracotta voor "Afgewezen (na gesprek)". Gebouwd op branch `stap4-eindbeslissing` — nog niet naar `main` gemerged; migratie moet na mergen nog handmatig via de Supabase SQL Editor worden uitgevoerd. |
| v1.52 | 30-08-2026 | Testpush op nieuwe branch `stap4-eindbeslissing` om te verifiëren dat de Netlify branch deploy werkt, nu de branchnaam is toegevoegd aan de lijst met deploybare branches. Geen inhoudelijke wijziging. |
| v1.51 | 30-08-2026 | Twee navigatieknoppen toegevoegd naast "Opslaan" onderaan het Toetsingsdocument: "← Terug naar aanmelding" (naar `aanmeldingen/[id]`) en "← Terug naar overzicht" (naar `aanmeldingen`), rechts van Opslaan in dezelfde `.btn-row`. Opslaan-gedrag zelf ongewijzigd. Gebouwd op branch `stap3-toetsing-herstructurering`. |
| v1.50 | 30-08-2026 | Weergave-fixes op het Toetsingsdocument na livetest (CC-opdracht "Toetsingsdocument: weergave-fixes", `CC-opdracht_Toetsingsdocument_Weergavefixes-2.docx`, geen data-/schemawijziging). Woonplaats onder de paginatitel is nu een H3 (was platte tekst). Basisgegevens staat nu in twee kolommen naast elkaar (Datum gesprek/Geboortedatum/KvK-nummer/Niveau links, Verzekering geldig tot/KOR-velden rechts) met `max-width: 320px` per veld i.p.v. de volle paginabreedte (`.toetsing-basisgegevens-grid`, stapelt op mobiel). Checklist (8 items) is omgebouwd van gestapelde radiogroepen naar een compacte tabel — één labelkolom + Ja/Nee/Twijfel-kolommen met kopregel bovenaan (moskleurige header, wit, hoofdletters), hergebruik van de bestaande rij-striping/hover van `admin-aanmeldingen-tabel`/`-rij-a`/`-rij-b`. De 14 open-vraagvelden (Vakinhoudelijk/Houding & cultuur/Energie) zijn qua hoogte gehalveerd (`.form-textarea--klein`, 55px i.p.v. 110px) — waren bedoeld voor korte aantekeningen, niet lange lappen tekst. Gebouwd op dezelfde branch als de herstructurering zelf (`stap3-toetsing-herstructurering`, v1.48) — nog niet naar `main` gemerged. |
| v1.49 | 30-08-2026 | Testpush op branch `stap3-toetsing-herstructurering` om te verifiëren dat de Netlify branch deploy voor deze branchnaam werkt, vóórdat de migratie op de gedeelde database wordt uitgevoerd. Geen inhoudelijke wijziging. |
| v1.48 | 30-08-2026 | Toetsingsdocument volledig geherstructureerd (CC-opdracht "Toetsingsdocument Stap 3, volledige herstructurering", `CC-opdracht_Toetsingsdocument_Stap3_Herstructurering.docx` — vervangt v1.42 integraal, niet als diff). Aanleiding: na livetest bleek de opzet niet praktisch tijdens een gesprek (geen overzicht van de eigen antwoorden van de docent, onlogische volgorde, placeholder-tekst die verdween tijdens typen). Nieuwe paginavolgorde: 1) Basisgegevens (incl. 2 nieuwe KOR-velden en Niveau, enige nog overschrijfbare veld), 2) nieuw read-only blok "Docent's eigen antwoorden" (7 velden rechtstreeks uit `aanmeldingen`, geen opslag in `toetsingen`) — opleiding/yogastijlen/andere_disciplines zijn hiermee niet langer overschrijfbaar en worden niet meer teruggeschreven naar `aanmeldingen` (dat was tot nu toe wel zo, zie v1.42), 3) Checklist (nu 8 items — `verzekering_in_orde` verhuisd van los BOOLEAN-veld naar TEXT-checklistitem, `opleiding_in_orde`/`certificaten_besproken` vervallen), 4) Open vragen (14 losse velden in 3 secties i.p.v. 5 grote vrije-tekstvelden — elke vraag nu een permanent zichtbaar label i.p.v. een placeholder die verdween tijdens typen), 5) vaste Startend/Ervaren-referentietekst (ongewijzigd, alleen verplaatst), 6) Afsluiting. Migratie `20260830090000_toetsingen_herstructurering.sql` bouwt met ALTER-statements voort op de bestaande tabel (bewust géén nieuwe CREATE TABLE) — de opdracht ging er nog van uit dat `toetsingen` niet op een gedeelde database stond, maar die stond al live (zie v1.45–v1.47); reeds ingevoerde testdata op de vervallen kolommen gaat daarom verloren, bewust aanvaard. Opslaan-gedrag, toegankelijkheid (alleen bij `match_beslissing = 'ja'`) en de letterlijke referentietekst zelf: ongewijzigd, conform sectie 5 van de opdracht. Gebouwd op branch `stap3-toetsing-herstructurering` — nog niet naar `main` gemerged; migratie moet na mergen nog handmatig via de Supabase SQL Editor worden uitgevoerd. |
| v1.47 | 29-08-2026 | Correctie op v1.45/v1.46: de werkelijke oorzaak van de wisselende Toetsingsdocument-knop was **Netlify Split Testing** (per ongeluk actief gezet, onbedoeld), die verzoeken willekeurig over twee verschillende deploys verdeelde — geen caching-probleem. Sluitend bevestigd doordat het patroon direct verdween (0 van 12 verzoeken misten de knop, was steeds ~50%) zodra Split Testing werd uitgezet, terwijl de `no-store`-fix uit v1.46 daarvoor zelf géén enkel verschil had gemaakt (nog steeds exact hetzelfde ~50%-patroon erna). De `Netlify Durable`-cache-header uit v1.45/v1.46 was dus een toevallig meelopend signaal (waarschijnlijk gekoppeld aan welke van de twee deploys antwoordde), geen echte oorzaak. De code-wijzigingen uit v1.45 (`force-dynamic`) en v1.46 (`no-store` op Supabase-fetches) blijven staan — geen schadelijke bijwerkingen, en een redelijke, behoudende keuze voor een admin-tool waar verse data zwaarder weegt dan cachingvoordeel — maar losten dít specifieke probleem dus niet op. |
| v1.46 | 29-08-2026 | Vervolg op v1.45: `export const dynamic = "force-dynamic"` op de admin-pagina's bleek onvoldoende — herhaalde `curl`-metingen (10+ opeenvolgende verzoeken) lieten een sluitend patroon zien: zodra de responseheader een aparte `cache-status: "Netlify Durable"`-regel bevatte (naast de reguliere Netlify Edge-laag), ontbrak de Toetsingsdocument-knop; zonder die regel stond hij er wél, telkens opnieuw reproduceerbaar. Conclusie: Netlify's "Durable"-cachelaag cachet blijkbaar de onderliggende Supabase-fetch-calls zelf, ongeacht de dynamic-instelling van de aanroepende Next.js-route. Opgelost op fetch-niveau i.p.v. route-niveau: `createServerClient()` (`src/lib/supabase/server.ts`) geeft nu een custom `global.fetch` mee aan de Supabase-client die altijd `cache: "no-store"` afdwingt op elke REST-aanroep. Dit geldt voor alle gebruik van `createServerClient()` in de hele codebase (ook `src/lib/docenten.ts`, de publieke docentenlijst) — een bewust conservatieve, sitebrede keuze: bij de huidige schaal weegt altijd-verse data zwaarder dan het (nog niet bestaande) prestatievoordeel van caching. Direct op `main` gefixt, zelfde reden als v1.45. |
| v1.45 | 29-08-2026 | Bugfix na livegang van Stap 3: de nieuwe "Toetsingsdocument"-knop op de detailpagina (v1.42) verscheen na de merge naar `main` niet consistent op de productiesite, ook niet na een "Deploy project without cache" — verse `curl`-tests lieten wisselende resultaten zien (knop soms wel, soms niet in de HTML-response), met een aparte `cache-status: "Netlify Durable"`-laag zichtbaar in de headers naast de reguliere Netlify Edge-cache. Oorzaak: deze admin-pagina's zijn weliswaar per request dynamisch gerenderd (`ƒ` in de build-output), maar zonder expliciete `dynamic`/`revalidate`-configuratie kan Netlify's Next.js-runtime een eerder gerenderde snapshot per URL (dus per aanmelding-`id`) blijven vasthouden in een cachelaag die bewust *over* deploys heen blijft bestaan — een gewone (of zelfs cache-loze) herdeploy lost dit daarom niet vanzelf op. Opgelost door `export const dynamic = "force-dynamic"` en `export const revalidate = 0` toe te voegen aan alle drie de admin-aanmeldingen-pagina's (`aanmeldingen/page.tsx`, `aanmeldingen/[id]/page.tsx`, `aanmeldingen/[id]/toetsing/page.tsx`), zodat deze routes nooit gecachet worden — passend bij hun aard (admin-tool met continu wijzigende beslissingsstatus). Direct op `main` gefixt i.p.v. via een feature-branch, omdat het een actief zichtbaar productieprobleem betrof. |
| v1.44 | 29-08-2026 | Nieuw open bouwpunt vastgelegd bij `ervaringsniveau` in het `docenten`-schema: niveau-wijziging (Startend → Ervaren) van een reeds actieve docent gebeurt na livegang via een simpele actie in het admin-dashboard (bijv. knop "Niveau aanpassen"), los van het Toetsingsdocument (tabel `toetsingen`, Stap 3) — dat blijft een historische vastlegging van het oorspronkelijke kennismakingsgesprek en wordt niet heropend voor latere niveauwijzigingen. Fase 4 punt 15 in de Bouwvolgorde uitgebreid met dezelfde losse actie, zodat dit niet wordt gemist bij het bouwen van het admin-dashboard. Puur documentatie — geen schema- of codewijziging. Direct op `main` gecommit (geen bouw-impact). |
| v1.43 | 29-08-2026 | Testpush op branch `stap3-toetsingsdocument` om te verifiëren dat de Netlify branch deploy voor deze (nieuwe) branchnaam werkt, nadat deze expliciet is toegevoegd aan de lijst met deploybare branches (Site settings → Build & deploy → Branches) — zelfde stap als destijds bij `stap2-admin-dashboard`. Geen inhoudelijke wijziging. |
| v1.42 | 29-08-2026 | Toetsingsdocument gebouwd (CC-opdracht "Toetsingsdocument — Stap 3", `CC-opdracht_Toetsingsdocument_Stap3-3.docx`): nieuwe pagina `app/dashboard/admin/aanmeldingen/[id]/toetsing/page.tsx` waarmee Sabine tijdens/na het kennismakingsgesprek een structureel toetsingsformulier invult, met formuliercomponent `ToetsingForm.tsx` en route `api/admin/aanmeldingen/[id]/toetsing` (upsert). Nieuwe tabel `toetsingen` (1-op-1 met `aanmeldingen`, unieke constraint op `aanmelding_id`, migratie `20260829210000_toetsingen.sql`) — geen RLS, zelfde reden als bij `aanmeldingen`: uitsluitend via `createServerClient()` (service-role key) benaderd. Pagina alleen bereikbaar/gelinkt vanaf de detailpagina bij `match_beslissing = 'ja'` (nieuwe knop "Toetsingsdocument invullen/bekijken" in `aanmeldingen/[id]/page.tsx`); rechtstreeks bezoeken van de URL bij een andere status geeft `notFound()`. Formulier heeft geen verplichte velden en geen afrond/vergrendel-status — blijft altijd te openen en te overschrijven. Vaste referentietekst (Startend/Ervaren-checklist) letterlijk overgenomen uit de opdracht, read-only. ⚠️ Interpretatie op één punt waar de opdracht niet volledig eenduidig was: sectie 4.1 noemt 4 "vooraf ingevuld, overschrijfbare" velden (opleiding, yogastijlen, extra specialisaties, niveau-inschatting), maar de gegeven `CREATE TABLE toetsingen` bevat alléén een kolom voor niveau (`niveau_definitief`) — geen kolommen voor de andere drie. De opdracht schrijft expliciet alleen voor `niveau_definitief` een terugschrijf-regel naar `aanmeldingen.niveau_inschatting` voor. Conform de gekozen aanpak: alle vier de velden op de toetsingspagina lezen/schrijven rechtstreeks van/naar de bestaande kolommen op `aanmeldingen` zelf (`opleiding`, `yogastijlen`, `andere_disciplines`, `niveau_inschatting`) — geen aparte/dubbele kolommen in `toetsingen`. Dit voorkomt zowel drijvende duplicaatvelden als stilzwijgend verlies van bewerkingen (er was namelijk geen kolom om ze anders in op te slaan). Nieuwe migratie is, net als eerdere, nog niet toegepast op de live database — moet nog handmatig via de SQL Editor worden uitgevoerd. Gebouwd op branch `stap3-toetsingsdocument` — nog niet naar `main` gemerged. |
| v1.41 | 29-08-2026 | Nieuw open bouwpunt vastgelegd in `20260820183919_init_schema.sql`, direct na `CREATE TABLE docenten` (zelfde stijl als het eerdere, inmiddels opgeloste bouwpunt bij `aanmeldingen`): een docentprofiel zou — net als bij `avb_document_url` — ook niet live moeten kunnen zonder geüpload bewijs van yoga-certificering (voorstel: kolom `certificaat_document_url`). Certificaten worden tijdens het kennismakingsgesprek (Stap 3) alleen besproken, niet geverifieerd; verificatie volgt pas bij het aanmaken van het profiel (Fase 4, nog niet gebouwd). Op verzoek van Sabine ook als bouwinstructie vastgelegd — niet alleen als schema-commentaar: sectie "6. Docentprofiel aanmaken" uitgebreid met dezelfde verplichting als de AVB-upload (`certificaat_document_url` verplicht vóór indienen ter goedkeuring), en Fase 4 punt 14 in de Bouwvolgorde aangevuld, zodat dit niet wordt gemist zodra de profielaanmaak-flow daadwerkelijk gebouwd wordt. Puur documentatie — geen schemawijziging of codewijziging in deze stap. |
| v1.40 | 29-08-2026 | Definitieve Calendly-link ingevuld: `CALENDLY_LINK_PLACEHOLDER` in `aanmeldingBeoordeling.ts` stond sinds v1.31 nog op `"#"` (afwachting van Sabine) — nu `https://calendly.com/privateyogaathome-info/30min?back=1&month=2026-08`, door Sabine zelf aangeleverd. Constante is (bewust) niet hernoemd, om de wijziging tot alleen de waarde te beperken. HTML-versie escaped de link nu via de bestaande `escapeHtml()`-helper (was ongeëscaped in de `href`, correct voor `"#"` maar niet meer voor een URL met een `&`). ⚠️ Te melden aan Sabine: de meegegeven link bevat `month=2026-08` — vermoedelijk een automatisch toegevoegd Calendly-queryparameter van het moment van kopiëren (initiële kalenderweergave), geen bewuste permanente instelling; kan op termijn een gedateerde eerste kalenderweergave geven voor wie de link later opent. Niet zelf aangepast, alleen letterlijk overgenomen zoals aangeleverd. Ontwikkeld op branch `stap2-admin-dashboard`, conform de in v1.39 opgezette workflow — nog niet naar `main` gemerged. |
| v1.39 | 29-08-2026 | Branch-workflow opgezet voor het vervolg van Stap 2 (CC-opdracht "Branch-workflow opzetten voor Stap 2", `CC-opdracht_Branch-workflow_Stap2.docx`) — reden: Netlify's productie-credits waren op voor deze billing cycle, en elke push naar `main` triggert een production deploy (15 credits), terwijl branch deploys/deploy previews gratis zijn. Nieuwe branch `stap2-admin-dashboard` aangemaakt vanaf `main` en gepusht; in Netlify (Site settings → Build & deploy → Branches) is "Let me add individual branches" ingesteld met `stap2-admin-dashboard` toegevoegd (i.p.v. "All", om ook toekomstige branches niet automatisch te laten deployen) — "Production branch" blijft `main`. Vanaf nu gaan commits/pushes voor de rest van Stap 2 naar deze branch i.p.v. naar `main`; deze changelog-regel is tegelijk de eerste testpush om te verifiëren dat de branch deploy in Netlify werkt. Mergen naar `main` (en daarmee de eerstvolgende production deploy) gebeurt pas als bewuste, losse stap na akkoord van Sabine, vermoedelijk na de credit-reset op 2 september. Geen wijzigingen aan bestaande functionaliteit, `.env.local` of de Supabase/Mollie-configuratie, conform de scope van de opdracht. |
| v1.38 | 29-08-2026 | Tekstcorrectie in `wachtlijstEmail()` (`aanmeldingBeoordeling.ts`, `text` én `html`): "...zodra we uitbreiden naar een nieuwe regio." → "...zodra we uitbreiden naar jouw regio." Verder niets aan deze mail of de andere templates gewijzigd, op verzoek van Sabine. |
| v1.37 | 29-08-2026 | Inhaalmigratie voor de in v1.36 gemelde bijvangst: nieuwe migratie `20260829120000_aanmeldingen_historische_inhaalslag.sql` legt de kolommen en constraints vast die sinds v1.29 (21-08-2026) al live op `aanmeldingen` staan maar nooit in een migratiebestand hadden gestaan (`yogastijlen`, `andere_disciplines`, `motivatie`, `toelichting`, `regio` + CHECK, de vier `akkoord_*`-kolommen, en de typewijziging van `jaren_leservaring` van INTEGER naar TEXT + CHECK). Niet op aannames gebaseerd: geverifieerd tegen de daadwerkelijke live database via een door Sabine gedraaide `pg_constraint`-query, waaruit ook bleek dat de CHECK-constraint op `jaren_leservaring` afwijkend is genoemd (`jaren_leservaring_check`, zonder tabelprefix) t.o.v. de overige constraints op deze tabel — nu letterlijk zo vastgelegd. Migratie is bewust idempotent (`ADD COLUMN IF NOT EXISTS`, conditionele `DO`-block) zodat ze geen foutmelding geeft als hij per ongeluk alsnog tegen de huidige (al bijgewerkte) database wordt gedraaid — in tegenstelling tot de eerdere migraties in dit project is deze dus juist niet bedoeld om nu nog te worden uitgevoerd, alleen om de repository weer een kloppende weergave van de live database te laten zijn. Verouderd "OPEN BOUWPUNT"-commentaar in `20260820183919_init_schema.sql` (beschreef deze velden nog als "nog niet gebouwd") bijgewerkt naar een verwijzing naar deze inhaalmigratie. |
| v1.36 | 29-08-2026 | Derde beslisknop "Wachtlijst" gebouwd (CC-opdracht "Wachtlijst-optie — Stap 2", `CC-opdracht_Wachtlijst.docx`). Vooraf bevestigd (sectie 0 van de opdracht): er bestaat al een technische regio-indicator — kolom `regio` op `aanmeldingen` (`haarlem_eo` / `wachtlijst`, ingevuld door de docent zelf via een verplichte radio-keuze op het aanmeldformulier, sinds v1.29) — dus geen handmatige beoordeling op het vrije `woonplaats`-veld nodig. Kolom `match_beslissing` uitgebreid met de waarde `'wachtlijst'` (migratie `20260829090000_aanmeldingen_wachtlijst.sql`, drop+recreate van de check-constraint). Detailpagina toont nu drie altijd-zichtbare knoppen (`btn-light` / `btn-dark-a` / `btn-dark-b`) — "Wachtlijst" vereist, net als afwijzen, geen niveau-inschatting; er wordt niets automatisch verborgen op basis van regio, Sabine beslist per geval. Nieuwe `wachtlijstEmail()` in `aanmeldingBeoordeling.ts`, zelfde gedeelde HTML-sjabloon/handtekening als de bestaande twee mails. Overzichtspagina: vierde statuslabel "Wachtlijst" (`statusLabel()` in `aanmeldingenBeoordeling.ts`, nieuwe badge-kleur `zacht`/`diep`); kolomkop "Woonplaats" is nu klikbaar en sorteert alfabetisch op woonplaats (nogmaals klikken keert de volgorde om, via `?sort=woonplaats&dir=asc/desc` — overschrijft zolang actief de standaard status-groepering, nieuwe helper `sorteerOpWoonplaats()`); aanmeldingen met `regio = 'wachtlijst'` krijgen op zowel de rij in het overzicht als op de detailpagina een klein "Buiten regio"-label (`.admin-badge-regio`, buiten scope: geen wijziging aan het aanmeldformulier of `REGIO_OPTIES` zelf). ⚠️ Bijvangst tijdens sectie-0-onderzoek: de kolommen `regio`, `yogastijlen`, `andere_disciplines`, `motivatie`, `toelichting` en de vier `akkoord_*`-verklaringen (allemaal sinds v1.29 in gebruik en live functionerend) blijken **nooit in een migratiebestand te hebben gestaan** — alleen `trainingsuren` (v1.30) en de beoordelingsvelden (v1.31) hebben een migratie in `supabase/migrations/`. Vermoedelijk destijds rechtstreeks via de SQL Editor toegevoegd zonder bijbehorend migratiebestand. Functioneel geen probleem zolang de live database ongemoeid blijft, maar bij het ooit opnieuw opbouwen van de database vanuit `supabase/migrations/` zouden deze kolommen ontbreken. Niet met terugwerkende kracht hersteld in deze opdracht (buiten scope) — gemeld zodat Sabine kan beslissen of dit alsnog wordt ingehaald met een aanvullende migratie. |
...
\-- OPEN BOUWPUNT: niveau-wijziging (Startend → Ervaren) na livegang gebeurt hier,  
\-- via een simpele actie in het admin-dashboard (bijv. knop "Niveau aanpassen").  
\-- Geen koppeling met het Toetsingsdocument (tabel \`toetsingen\`) — dat blijft een  
\-- vastlegging van het oorspronkelijke kennismakingsgesprek en wordt niet  
\-- heropend voor latere niveauwijzigingen. Zie CLAUDE.md Bouwvolgorde, Fase 4, punt 15.  
...
\- **Certificaat-upload is eveneens een verplicht veld** — het profiel kan niet worden ingediend ter goedkeuring zonder geüpload bewijs van yoga-certificering  
\- Upload verloopt via Cloudinary, net als de AVB-upload  
\- Het geüploade document wordt opgeslagen als \`certificaat\_document\_url\` in de \`docenten\` tabel  
\- Certificaten worden tijdens het kennismakingsgesprek (Stap 3) alleen besproken, niet geverifieerd — Sabine controleert het document pas bij profielgoedkeuring  
\- Zolang \`certificaat\_document\_url\` leeg is, kan het profiel niet op \`actief = true\` worden gezet
...
14\. Docent dashboard (profiel bewerken, aanvragen zien) — inclusief verplicht AVB-uploadveld én verplicht certificaat-uploadveld; profiel kan niet worden ingediend zonder geüpload verzekeringsbewijs en geüpload bewijs van yoga-certificering  
15\. Admin dashboard (docenten goedkeuren incl. niveau toewijzen, AVB-document inzien en goedkeuren, betalingen overzicht), inclusief een losse actie om het niveau van een reeds actieve docent aan te passen (Startend ↔ Ervaren) — los van het Toetsingsdocument, dat historisch blijft en niet wordt heropend voor dit doel.
